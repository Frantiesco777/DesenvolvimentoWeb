...
$sql = "SELECT * FROM animes_geral WHERE genero='romance' ORDER BY criado_em DESC";
...
<head>
  ...
  <title>Animes Romance - AnimaXone</title>
...
<body>
  ...
  <section>
    <h2 class="titulo-sessao">Gênero: Romance</h2>
    <div class="animes_assistindo">
      <?php if (count($animesRomance) > 0): ?>
        <?php foreach($animesRomance as $anime): ?>
        ...
        <?php endforeach; ?>
      <?php else: ?>
        <p style="color: #ccc; text-align: center;">Nenhum anime de Romance encontrado.</p>
      <?php endif; ?>
    </div>
  </section>
