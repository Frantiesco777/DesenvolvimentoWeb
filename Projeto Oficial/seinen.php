...
$sql = "SELECT * FROM animes_geral WHERE genero='seinein' ORDER BY criado_em DESC";
...
<head>
  ...
  <title>Animes Seinein - AnimaXone</title>
...
<body>
  ...
  <section>
    <h2 class="titulo-sessao">Gênero: Seinein</h2>
    <div class="animes_assistindo">
      <?php if (count($animesSeinein) > 0): ?>
        <?php foreach($animesSeinein as $anime): ?>
        ...
        <?php endforeach; ?>
      <?php else: ?>
        <p style="color: #ccc; text-align: center;">Nenhum anime de Seinein encontrado.</p>
      <?php endif; ?>
    </div>
  </section>
